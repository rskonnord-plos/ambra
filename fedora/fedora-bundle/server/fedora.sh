...
AXIS_UTILITY_LIBS=$TC_COMMON/activation-1.0.2.jar:$TC_COMMON/axis.jar:$TC_COMMON/commons-discovery.jar:$TC_COMMON/commons-logging.jar:$TC_COMMON/jaxrpc.jar:$TC_COMMON/mail.jar:$TC_COMMON/saaj.jar:$TC_COMMON/tt-bytecode.jar:$TC_COMMON/wsdl4j-1.5.1.jar:$TC_COMMON/xercesImpl.jar:$TC_COMMON/xml-apis.jar
SERVER_LIBS=$TC_COMMON/log4j-1.2.8.jar
...
start() {
	...
	if [ -z "${SERVER_PROFILE}" ]; then
  	...
  	(exec nohup "$JAVA" -server -Xmn64m -Xms256m -Xmx256m \
		...
		org.apache.catalina.startup.Bootstrap start & echo $! > $PIDFILE)
	...
	(exec "$JAVA" -classpath "$TC"/webapps/fedora/WEB-INF/classes:"$SERVER_LIBS" \
		...
		-Dfedora.home="$FEDORA_HOME" \
		-Dtomcat.dir="$TC_BASENAME" \
		fedora.server.utilities.status.ServerStatusTool watch-startup 90)
