 ...
 * Modified to suit the needs of PLoS ONE
 * 
 * @author David M Johnson
 */
public class LinkbackExtractor {
  private static Log log = LogFactory.getLog(LinkbackExtractor.class);
   ...
   * @throws IOException
   */
  public LinkbackExtractor(String refererURL, String requestURL) throws MalformedURLException,
                                                                        IOException {
    try {
      extractByParsingHtml(refererURL, requestURL);
    } catch (Exception e) {
      if (log.isErrorEnabled()) {
        log.error("Error extracting linkback for refererURL: " + refererURL + 
                  " and requestURL: " + requestURL, e);
      }
    }
  }
  ...
  /**
   * Parser callback that finds title and excerpt. As we walk through the HTML tags, we keep track
   * of the most recently encountered divider tag in the mStart field. Once we find the referring
   * permalink, we set the mFound flag. After that, we look for the next divider tag and save it's
   * position in the mEnd field.
   */
  private final class LinkbackCallback extends ParserCallback {
    // Dividers
    private Tag[] mDivTags = { Tag.TD, Tag.DIV, Tag.SPAN, Tag.BLOCKQUOTE, Tag.P, Tag.LI, Tag.BR,
    ...
    public void handleStartTag(Tag tag, MutableAttributeSet atts, int pos) {
      if (mList.contains(tag) && !mFound) {
        ...
        if (href != null && (href.equals(mRequestURL) || href.equals(mRequestURLWWW))) {
          ...
          /*
           * if (mLogger.isDebugEnabled()) { mLogger.debug("No match: "+href); }
           */
        }
      }
      mCurrentTag = tag;
