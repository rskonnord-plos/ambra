<#--
  $HeadURL: http://svn.ambraproject.org/svn/plos/templates/head/journals/PLoSDefault/webapp/article/browseIssue.ftl $
  $Id: browseIssue.ftl 12939 2012-11-30 22:43:21Z josowski $
  ...
  <div class="related">
    ...
    <ul>
      <#list articleInfo.relatedArticles as ra>
        <#assign docURL = "${freemarker_config.doiResolverURL}${ra.uri?replace('info:doi/','')}">
        <li><a href="${docURL}"
               title="Read Article"><@articleFormat>${ra.title}</@articleFormat></a></li>
      </#list>
    </ul>
  </div>
  </#if>
              ...
              <div class="item cf">
                ...
                <#if (articleGrp.heading == "Research Article") >
                  ...
                  <#--ignore article xml and pdf-->
                    <#if (articleInfo.articleAssets?size > 2) >
