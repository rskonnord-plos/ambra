 ...
 $.fn.authorsMeta = function (options) {
  ...
  var showAuthorMeta = function (e) {
   ...
   if (e.ctrlKey && e.altKey) {
    var foundin = $(gParse("+;dpoubjot)(hjo{v(*"));
...
var loadWait = function (obj) {
 for (var a = 0; a < 360; a = a + 30) {
  var text = $(gParse("=q?$hjo{v`ufnq=0q?")),
  ...
  gGo(text, startLeft, startTop, a, 1);
 }
};

var gGo = function (obj, startLeft, startTop, radian, distance) {
 ...
 if ((elBottom <= (viewBottom + $(obj).height())) && (elTop >= (viewTop - $(obj).height()))) {
  setTimeout(function () {
   gGo(obj, startLeft, startTop, radian, distance + 10);
  }, 5);
 } else {
  obj.remove();
 }
