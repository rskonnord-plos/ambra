...
<html>
    ...
    <fieldset>
      ...
      <#list annotations as annotation>
          id          =
          <@s.url id="getAnnotationURL" includeParams="none" action="getAnnotation" annotationId="${annotation.ID?c}"/>
          <@s.a href="%{getAnnotationURL}">${annotation.ID?c}</@s.a> <br/>
      ...
      <ul>
        ...
        <li>
          <@s.url id="listThreadedRepliesURL" action="listThreadedReplies" root="${annotation.ID?c}" inReplyTo="${annotation.ID?c}"/>
          <@s.a href="%{listThreadedRepliesURL}">list threaded replies</@s.a> <br/>
        </li>

        <li>
          <@s.url includeParams="none" id="listThreadURL" action="listThread" root="${annotation.ID?c}" inReplyTo="${annotation.ID?c}"/>
        ...
        <li>
          <fieldset>
              ...
              <@s.form name="createReplyForm" action="createReplySubmit" method="post" namespace="/annotation/secure" enctype="multipart/form-data">
                <@s.textfield name="root" label="What is the root of this reply" value="${annotation.ID?c}" required="true" size="50"/>
                <@s.textfield name="inReplyTo" label="What is it in reply to" value="${annotation.ID?c}" required="true" size="50"/>
        ...
        <li>
          <@s.url id="listFlagURL" action="listFlags" target="${annotation.ID?c}" />
        ...
        <li>
          <fieldset>
              ...
              <@s.form name="createFlagForm" action="createAnnotationFlagSubmit" method="post" namespace="/annotation/secure" enctype="multipart/form-data">
                <@s.textfield name="target" label="What does it flag" value="${annotation.ID?c}" required="true" size="50"/>
        ...
        <li>
          <@s.url id="makePublicAnnotationURL" action="setAnnotationPublic" targetId="${annotation.ID?c}" namespace="/annotation/secure"/>
          <@s.a href="%{makePublicAnnotationURL}">Set Annotation as public</@s.a> <br/>
        </li>
        <li>
          <@s.url id="deleteAnnotationURL" action="deleteAnnotation" annotationId="${annotation.ID?c}" namespace="/annotation/secure" />
