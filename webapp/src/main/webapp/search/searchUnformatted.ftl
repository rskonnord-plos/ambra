          ...
          <#if journals??>
            <#list journals as journal>
              <option id="filterJournalsPicklist_${journal.name}"
                      ...
                      title="${freemarker_config.getDisplayName(journal.name)}">${freemarker_config.getDisplayName(journal.name)}</option>
            </#list>
          </#if>
          </select>
        </div>
        <ol class="text">
          <li style="margin:0.5em 0;"><label for="volumeId">Volume:</label><input type="text" name="volume"
                                                                                  id="volumeId"
                                                                                  value="${volume?html!""}"
                                                                                  title="Volume"
                                                                                  alt="Volume Text Field"/><span
              class="example" style="margin-left:1em;">e.g.: 3</span></li>
          <li><label for="eLocationIdId">eNumber:</label><input type="text" name="eLocationId" id="eLocationIdId"
                                                                value="${eLocationId?html!""}" title="eNumber"
                                                                alt="eNumber Text Field"/><span class="example"
                                                                                                style="margin-left:1em;">e.g.: e2243</span>
          </li>
        </ol>
      </fieldset>
      <fieldset>
        ...
        <ul class="text">
          <li><label for="idId">Article DOI:</label><input type="text" name="id" id="idId" size=17" value="${id?html!""}
            " title="Article DOI" alt="Article DOI Text Field"/><span class="example" style="display:block;">e.g.: 10.1371/journal.pone.0002243</span>
          </li>
        </ul>
      </fieldset>
      <div class="btnwrap">
...
<fieldset id="queryBuilder">
    ...
    <select name="queryField" id="queryFieldId" title="Search Field" alt="Search Field Pick List">
      ...
      <option value="volume" title="Volume Number">Volume Number</option>
          ...
          <#if query?? && query?length gt 0>
            <input type="text" name="queryTerm" id="queryTermId" onblur="if(this.value=='')value='Enter search terms';"
                   onfocus="if(this.value=='Enter search terms')value='';" value="${query}" title="Search Term"
                   alt="Search Term Text Field"/>
          <#else>
            <input type="text" name="queryTerm" id="queryTermId" onblur="if(this.value=='')value='Enter search terms';"
                   onfocus="if(this.value=='Enter search terms')value='';" value="${"Enter search terms"}"
                   title="Search Term" alt="Search Term Text Field"/>
          </#if>
          </span>
          <span id="startAndEndDateDivBlockId" style="display:none">
            <input type="text" name="startDateAsString" maxlength="10" value="${startDateAsString!"YYYY-MM-DD"}"
                   id="startDateAsStringId" disabled="true" title="Start Date" alt="Start Date Text Field"/>
            to
            <input type="text" name="endDateAsString" maxlength="10" value="${endDateAsString!"YYYY-MM-DD"}"
                   id="endDateAsStringId" disabled="true" title="End Date" alt="End Date Text Field"/>
          </span>
  </div>
  ...
  <div style="margin:5px 0;">
    <textarea name="unformattedQuery" id="unformattedQueryId" title="Query"
              alt="Query Text Area">${unformattedQuery?html}</textarea>
  </div>
  <div class="btnwrap">
    ...
    <input type="button" name="clearUnformattedQueryButton" id="clearUnformattedQueryButtonId" value="Clear Query"
           title="Clear Query" alt="Clear Query Button"/>
  </div>
</fieldset>
...
<fieldset id="journals">
  ...
  <ol>
    <li><label><input id="journalsOpt_all" type="radio" name="journalOpt" value="all"<#if (filterJournals?size == 0)>
                      checked="true"</#if> title="Search All Journals" alt="Search All Journals Radio Button"/> Search
      all journals</label></li>
    <li><label><input id="journalsOpt_slct" type="radio" name="journalOpt"
                      value="some"<#if (filterJournals?size gt 0)> checked="true"</#if>
                      title="Search Selected Journals" alt="Search Selected Journals Radio Button"/> Only search in
      the following journals:</label></li>
    <li class="options">
      <fieldset id="fsJournalOpt">
        <ul>
        <#if journals??>
          <#list journals as journal>
            <li><input id="filterJournals_${journal.name}" name="filterJournals" value="${journal.name}"
                       ...
                       alt="Select Journal ${freemarker_config.getDisplayName(journal.name)} Check Box"/>&nbsp;<label
                for="filterJournals_${journal.name}">${freemarker_config.getDisplayName(journal.name)}</label></li>
          </#list>
        <#else>
          <br/>
...
<fieldset id="subjCats">
  ...
  <ol>
    <li><label><input id="subjectOption_all" type="radio" checked="checked" name="subjectCatOpt"
                      value="all" <#if (filterSubjects?size == 0) > checked="true"</#if>
                      title="Search All Subject Categories" alt="Search All Subject Categories Radio Button"/> Search
      all subject categories</label></li>
    <li><label><input id="subjectOption_some" type="radio" name="subjectCatOpt"
                      value="some" <#if (filterSubjects?size gt 0)> checked="true"</#if>
                      title="Search Selected Subject Categories"
                      alt="Search Selected Subject Categories Radio Button"/> Only look for articles with the
      following subject categories:</label></li>
    <li class="options">
      <fieldset id="fsSubjectOpt">
      <#if (filterSubjects?size gt 0)><p>Listed below are all subject categories from <b
          title="Articles that already match your entered search terms">matching</b> articles.</p></#if>
        ...
        <ul>
          <#list subjects?sort_by("name") as subject>
            <#if (subject_index + 1) lte colSize>
              ...
              <li><input id="filterSubjects_${subjectId}" name="filterSubjects" value="${subject.name}"
                         type="checkbox" <#if (filterSubjects?seq_contains(subject.name)) > checked="true"</#if>
                         title="Select Subject Category ${subject.name}"
                         alt="Select Subject Category ${subject.name} Check Box"/>&nbsp;<label
                  for="filterSubjects_${subjectId}">${subject.name} (${subject.count})</label></li>
            </#if>
          </#list>
        </ul>
        <ul>
...
<fieldset id="subjCats">
  ...
  <ol>
    <li><label><input id="articleType_all" type="radio" checked="checked" name="filterArticleTypeOpt"
                      value="all" <#if (filterArticleType?length == 0)> checked="true"</#if>
                      title="Search All Article Types" alt="Search All Article Types Radio Button"/> Search all
      article types</label></li>
    <li><label><input id="articleType_one" type="radio" name="filterArticleTypeOpt"
                      value="some" <#if (filterArticleType?length gt 0)> checked="true"</#if>
                      title="Search For Only Selected Article Type"
                      alt="Search Only For Selected Article Type Radio Button"/> Search for one of the
      following:</label></li>
    <li class="options">
        ...
        <ul>
          <#list articleTypes?sort_by("name") as articleType>
            <#if (articleType_index + 1) lte colSize>
              ...
              <li><input id="filterArticleType_${articleTypeId}" name="filterArticleType" value="${articleType.name}"
                         type="radio" <#if (filterArticleType == articleType.name) > checked="true"</#if>
                         title="Select Article Type ${articleType.name}"
                         alt="Select Article Type ${articleType.name} Check Box"/>&nbsp;<label
                  for="filterArticleType_${articleTypeId}">${articleType.name}</label></li>
            </#if>
          </#list>
        </ul>
        <ul>
...
<div class="btnwrap">
  ...
  <input type="button" name="clearFiltersButton2" id="clearFiltersButtonId2" value="Clear Filters"
         title="Clear Filters" alt="Clear Filters"/>
</div>
