<#--
  $HeadURL: http://svn.ambraproject.org/svn/plos/templates/head/journals/PLoSDefault/webapp/includes/article_tabs.ftl $
  $Id: article_tabs.ftl 13852 2013-03-01 03:11:34Z mbaehr $
...
<#if !articleTabURL?? ><@s.url id="articleTabURL"  namespace="/article" action="fetchArticle"         includeParams="all"/></#if>
<#if !authorTabURL??  ><@s.url id="authorTabURL"   namespace="/article" action="fetchArticleAuthors"  includeParams="all"/></#if>
<#if !metricsTabURL?? ><@s.url id="metricsTabURL"  namespace="/article" action="fetchArticleMetrics"  includeParams="all"/></#if>
<#if !relatedTabURL?? ><@s.url id="relatedTabURL"  namespace="/article" action="fetchRelatedArticle"  includeParams="all"/></#if>
<#if !commentsTabURL??><@s.url id="commentsTabURL" namespace="/article" action="fetchArticleComments" includeParams="all"/></#if>

<#macro tabItem tabValue linkTarget linkText>
<li>
  <#if tab == tabValue>
    <#if isTabChildPage?? && isTabChildPage>
      <a href="${linkTarget}" class="active">${linkText}</a>
    <#else>
      <span class="active">${linkText}</span>
    </#if>
  <#else>
