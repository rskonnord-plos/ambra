...
public class AnnotationServiceTest extends BaseTest {
  ...
  {
    /**
     * Make sure first that the DB is in a clean state
     */
    return;
  }
  ...
  public Object[][] annotationIdList() throws Exception {
    ...
    article.setId(formalCorrection.getAnnotates());
    ...
    article1.setId(comment.getAnnotates());
  ...
  public Object[][] classConversionDataProvider() {
    ...
    article.setDublinCore(new DublinCore());
    article.getDublinCore().setBibliographicCitation(new Citation());
    article.getDublinCore().getBibliographicCitation().setId(URI.create("id://test-citation"));
    article.getDublinCore().setIdentifier("id://test-article");
    dummyDataStore.store(article.getDublinCore().getBibliographicCitation());
    dummyDataStore.store(article.getDublinCore());
