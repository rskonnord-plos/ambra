...
public class BaseWebTest extends BaseTest {
   ...
   * from the session, set up mock action invocations, etc.  To configure this to run before a unit test, add
   * <code>groups = {ADMIN_GROUP}</code> to the <code>@Test</code> annotation.
   */
  @BeforeGroups(groups = ADMIN_GROUP)
  ...
  /**
   * Set up struts container / context with a user request.  This is for unit tests that need to access attributes
   * from the session, set up mock action invocations, etc., To configure this to run before a unit test, add
   * <code>groups = {USER_GROUP}</code> to the <code>@Test</code> annotation
   */
  @BeforeGroups(groups = USER_GROUP)
