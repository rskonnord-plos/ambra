...
public class BrowseServiceTest {
  ...
  public void testGetArticleDates() throws Exception{
    ...
    expect(session.createQuery("select a.dublinCore.date from Article a where a.state = :st;")).andReturn(query);
    expect(query.setParameter("st", Article.STATE_ACTIVE)).andReturn(query);
