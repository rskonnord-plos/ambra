...
public class AnnotationCitationMigrator implements Runnable {
  private static final Log log = LogFactory.getLog(AnnotationCitationMigrator.class);
  ...
  public long migrate() {
    ...
    try {
      ...
      if (errorSet.size() > 0)
        log.fatal("Failed to migrateFormalCorrections " + errorSet.size() + " annotation citations. " +
    ...
    while (results.next()) {
      ...
      if (correction.getBibliographicCitation() == null) {
        ...
        if (errorSet.contains(id))
          continue;
        try {
          log.info("Migrating cittation for " + id);
          AnnotationService.createDefaultCitation(correction, articleCitation, session);
          session.flush();
          count++;
        } catch (Exception e) {
    ...
    while (results.next()) {
      ...
      if (retraction.getBibliographicCitation() == null) {
        ...
        try {
          ...
          AnnotationService.createDefaultCitation(retraction, articleCitation, session);
          session.flush();
          count++;
        } catch (Exception e) {
