...
public class HomePageAction extends BaseActionSupport {
  ...
  //properties for the "Most Viewed" section
  ...
  private List<URI> getArticleTypesToShow(String basePath) {
    ...
    private void initRecentArticles() {
      ...
      recentArticles = browseService.getArticlesByDate(startDate, endDate, typeUriArticlesToShow,
        0, numArticlesToShow * 100, new long[1]);
      ...
      if (recentArticles.size() < numArticlesToShow) {
        ...
        recentArticles.addAll(browseService.getArticlesByDate(startDate, endDate, typeUriArticlesToShow,
          0, numArticlesToShow - recentArticles.size(), new long[1]));
      }
      ...
      if (recentArticles.size() > 0) {
        ...
        List<SearchHit> recentArticlesTemp = new LinkedList<SearchHit>();
  ...
  private boolean mostViewedEnabled() {
    return configuration.containsKey("ambra.virtualJournals." + getCurrentJournal() + ".mostViewedArticles.limit") && solrHttpService != null;
  }
  ...
  private void initMostViewed() {
    ...
    try {
      ...
      mostViewedArticles = solrHttpService.getMostViewedArticles(getCurrentJournal(), limit, days);
