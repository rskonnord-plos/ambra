...
public class ArticleSyndicationHistory extends BaseAdminActionSupport {
  ...
  public String resyndicate() {
    try {
      if (articleURI != null) {
        if ((synTargets != null) &&( synTargets.length > 0)) {
          for(String target : synTargets)
            ...
            try {
              syndicationService.syndicate(articleURI.toString(), target);
            } catch (Exception e) {
              syndicationService.updateSyndication(
                  articleURI.toString(), target, Syndication.STATUS_FAILURE, e.getMessage());
              throw e;
            }
        } else {
      ...
      String err = "Syndication failed for DOI: ";
      if (articleURI != null)
        err = err +  articleURI.toString() + "<br/>";
      log.error(err, e);
      addActionError(err + e);
  ...
  public String markSyndicationAsFailed() {
    try {
      ...
      String err = "Could not mark Syndication as failed for DOI: ";
      if (articleURI != null)
        err = err +  articleURI.toString() + "<br/>";
      log.error(err, e);
      addActionError(err + e);
