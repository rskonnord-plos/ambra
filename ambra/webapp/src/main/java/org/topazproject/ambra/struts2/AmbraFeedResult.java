...
public class AmbraFeedResult extends Feed implements Result {
  ...
  private List<Entry> buildAnnotationFeed(String xmlBase,
    ...
    for (Annotea annot : map.values()) {
      ...
      List <Content> contents = newAnnotationsList(
          selfLink, annotationType, authors, getBody(annot));
  ...
  private List<Person> newContributorsList(Article article) {
    ...
    /*
     * FIXME: contributors field exists in DublinCore but it's not being populated on ingest.
     * article.getDublinCore().getContributors() is useless at the moment.
     */
    ...
    if (citation != null) {
      // Avoid duplicates
      contributors = new HashSet<String>(contributors);
      contributors.addAll(citation.getCollaborativeAuthors());
    }
  ...
  private String newAuthorsList(ArticleFeedCacheKey cacheKey, Article article, List<Person> authors) {
    ...
    if (bc != null) {
      List<ArticleContributor> authorProfiles = article.getAuthors();

      if (cacheKey.isExtended()) {
        /* If extended then create a list of persons
         * containing all the authors.
         */
        for (ArticleContributor profile : authorProfiles) {
          Person person = new Person();
  ...
  private Link newAltLink(Article article, Representation rep, String xmlBase) {
    ...
    if(includeformatting) {
      ...
      altLink.setTitle("(" + rep.getName() + ") " + TextUtils.simpleStripAllTags(dc.getTitle()));
    }
  ...
  private Link newSelfLink(Article article, String xmlBase) {
    ...
    if(article!=null){
      ...
      if(includeformatting) {
        ...
        link.setTitle(TextUtils.simpleStripAllTags(dc.getTitle()));
      }
    }
    return link;
  ...
  private Entry newEntry(Article article) {
    ...
    if(includeformatting) {
      ...
      entry.setTitle(TextUtils.simpleStripAllTags(dc.getTitle()));
    }
