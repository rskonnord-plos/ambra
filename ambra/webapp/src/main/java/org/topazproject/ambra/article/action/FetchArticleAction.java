...
import org.topazproject.ambra.annotation.service.*;
...
public class FetchArticleAction extends BaseSessionAwareActionSupport {
  ...
  private void setCommonData() throws ApplicationException, NoSuchArticleIdException {
    ...
    articleInfoX = browseService.getArticleInfo(URI.create(articleURI), getAuthId());
    ...
    isResearchArticle = articlePersistenceService.isResearchArticle(articleURI, getAuthId());
    ...
    Article artInfo = articlePersistenceService.getArticle(URI.create(articleURI), getAuthId());
    ...
    //Getting these properties so they are not lazy loaded
    c.getELocationId();
