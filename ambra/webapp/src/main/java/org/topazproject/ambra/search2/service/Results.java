...
public class Results {
  ...
  /**
   * Lookup the rest of the information for this Article (from the Article URI) and write a
   * selection of that data into the returned SearchHit object.
   *
   * @param hit the SearchHit which will have the rest of its content filled in by this method
   * @return An object fully populated with Article data
   */
  private SearchHit resolveHit(SearchHit hit) {
    ...
    // Get a displayable version of the Article Type by doing a lookup on every element
    // of the Set of all Article Type URIs for this Article.
    // The first successful lookup is used under that assumption that there is only one legit value.
    // Default to "Unclassified".  Never throw an exception, but do log "Unclassified" Articles.
    // Compare with method ArticleInfo.getArticleTypeForDisplay(), which does the same thing.
    String articleTypeForDisplay = "Unclassified";
    try {
      ArticleType articleType = null;
      for (URI artTypeUri : article.getArticleType()) {
      ...
      log.warn("The acquisition of a user-friendly Article Type failed for article " + uri, e);
    }
    ...
    try {
      return new SearchHit(hit.getHitScore(), hit.getUri(), article.getDublinCore().getTitle(),
                           ...
                           hit.getJournalTitle(), articleTypeForDisplay);
