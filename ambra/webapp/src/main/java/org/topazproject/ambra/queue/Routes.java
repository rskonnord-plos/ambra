...
public class Routes extends SpringRouteBuilder {
  ...
  public void configure() throws Exception {
    ...
    from("direct:test")
        .to("log:org.topazproject.ambra.queue.TestQueue?level=INFO" +
            "&showBodyType=false" +
            "&showBody=true" +
            "&showExchangeId=true" +
            "&multiline=true");
    ...
    if (searchIndexingQueue != null) {
      ...
      from(SEARCH_INDEX)
          .to("log:org.topazproject.ambra.queue.search.delete.MessageReceived?level=INFO" +
              "&showBodyType=false" +
              "&showBody=false" +
              "&showExchangeId=true")
    ...
    if (searchDeleteQueue != null) {
      ...
      // As set in default exception handler: redeliver forever
      from(SEARCH_DELETE)
          .to("log:org.topazproject.ambra.queue.search.index.MessageReceived?level=INFO" +
              "&showBodyType=false" +
              "&showBody=false" +
              "&showExchangeId=true")
    ...
    if (searchMailReceiver != null) {
      ...
      from(SEARCH_INDEXALL)
          .onException(Exception.class)
            ...
            .to("smtp://" + mailServer)
          .end()
          .to("log:org.topazproject.ambra.queue.search.indexall.MessageReceived?level=INFO" +
              "&showBodyType=false" +
              "&showBody=false" +
              "&showExchangeId=true")
          ...
          .setHeader("to", constant(searchMailReceiver))
          .setHeader("from", constant("do-not-reply@plos.org"))
          .setHeader("subject", constant("All articles indexed"))
          .to("smtp://" + mailServer);
    } else {
