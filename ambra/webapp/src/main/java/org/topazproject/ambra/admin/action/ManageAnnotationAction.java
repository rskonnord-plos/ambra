...
public class ManageAnnotationAction extends BaseAdminActionSupport {
  private static final Log log = LogFactory.getLog(ManageAnnotationAction.class);
  ...
  public String loadAnnotation() throws Exception {
    ...
    try {
      ...
      if (annotationBody == null)
        annotationBody = annotation.getEscapedComment();
      ...
      if (annotation.getCitation() != null) {
        ...
        if (annotation.getCitation().getAuthors() != null
            && annotation.getCitation().getAuthors().size() > 0) {
          citationAuthorIds = new String[annotation.getCitation().getAuthors().size()];
          citationAuthorGivenNames = new String[annotation.getCitation().getAuthors().size()];
          citationAuthorSurnames = new String[annotation.getCitation().getAuthors().size()];
          citationAuthorSuffixes = new String[annotation.getCitation().getAuthors().size()];
          int authorIndex = 0;
          for (UserProfile author : annotation.getCitation().getAuthors()) {
  ...
  public String saveAnnotation() throws Exception {
    annotationService.updateBodyAndContext(annotationId, annotationBody, annotationContext);
    ...
    if (citationId != null && citationId.trim().length() > 0) {
      ...
      if (citationAuthorSurnames.length > 1) {
        for (int i=0; i < citationAuthorSurnames.length - 1; i++) {
          citationService.updateAuthor(citationAuthorIds[i].trim(), citationAuthorSurnames[i].trim(),
      ...
      if (citationAuthorDeleteIndex > -1) {
        citationService.deleteAuthor(citationId, citationAuthorIds[citationAuthorDeleteIndex]);
      ...
      if (citationAuthorGivenNames[citationAuthorSurnames.length - 1].trim().length() > 0
        ...
        citationService.addAuthor(citationId,
