...
/**
 * Fetch the object is primarily used to fetch assets associated with a particular DOI.
 * These assets (tif's pdf's, supplementary data etc) are either used in the rendered html
 * version of the article or supplied to the user for downloading.
 *
...
public class FetchObjectAction extends BaseActionSupport {
  ...
  public String fetchObjectAction() {
    try {
      ...
      //WTO: Need to add XReproxy code here.
      inputStream = (fullDOI && "XML".equals(representation))
        ? xmlService.getTransformedInputStream(fsStream) : fsStream;

    } catch (Exception e) {
      log.error("Error retrieving object: " + uri, e.getMessage());
      return ERROR;
    }
    return SUCCESS;
  }
  ...
  /**
   * @return Return the list of x-reproxy url's that are available
   * for this request.
   */
