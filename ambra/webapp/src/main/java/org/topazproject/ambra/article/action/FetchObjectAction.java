...
public class FetchObjectAction extends BaseActionSupport {
  ...
  public String execute() throws Exception {
    ...
    ObjectInfo objectInfo = articlePersistenceService.getObjectInfo(uri);
    if (null == objectInfo) {
    ...
    if(fullDOI) {
      if(!"XML".equals(representation)) {
        addActionMessage("You can not run a full DOI XSL transformation on a non XML " +
            "representation");
        return ERROR;
      }
      handleBlob(rep, true);
    } else {
      handleBlob(rep, false);
    }
   ...
   * @throws Exception
   */
  private void handleBlob(final Representation rep, boolean fullDOI) throws Exception {
    ...
    if (rep.getSize() <= smallBlobSizeBytes) {
      if(fullDOI) {
        inputByteArray = fullDOIService.getTransformedByArray(smallBlobService.getSmallBlob(rep));
      } else {
      ...
      if(fullDOI) {
        inputStream = fullDOIService.getTransformedInputStream(rep.getBody().getInputStream());
      } else {
  ...
  public String fetchSingleRepresentation() throws Exception {
    final ObjectInfo objectInfo = articlePersistenceService.getObjectInfo(uri);
    ...
    handleBlob(representations.iterator().next(), false);
  ...
  /**
   * Set the XMLService
   * @param fullDOIService
   */
  @Required
  public void setFullDOIService(XMLService fullDOIService) {
    this.fullDOIService = fullDOIService;
  }
