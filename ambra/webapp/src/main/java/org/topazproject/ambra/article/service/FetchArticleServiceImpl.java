...
public class FetchArticleServiceImpl extends HibernateServiceImpl implements FetchArticleService {
  ...
  private static final String ARTICLE_LOCK = "ArticleAnnotationCache-Lock-";
  ...
  public String getURIAsHTML(final String articleURI, final String authId) throws Exception {
    ...
    articleService.getArticle(URI.create(articleURI), authId);
    
    final Object lock = (ARTICLE_LOCK + articleURI).intern();  // lock @ Article level

    String content = articleAnnotationCache.get(ARTICLE_KEY  + articleURI,
  ...
  private Document getAnnotatedContentAsDocument(final String articleDOI, final String authId)
    ...
    try {
      content = articleService.getContent(articleDOI, articleTransformService.getArticleRep(), authId);
  ...
  public Document getArticleDocument(String articleURI, final String authId) {
    ...
    try {
      content = articleService.getContent(articleURI, articleTransformService.getArticleRep(), authId);
    } catch (Exception e) {
  ...
  /**
   * @param articleAnnotationCache The Article(transformed) cache to use
   */
  @Required
