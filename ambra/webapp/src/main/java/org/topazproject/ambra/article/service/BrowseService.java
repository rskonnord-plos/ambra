...
public class BrowseService {
   ...
   * <br/>
   * It is necessary to query for <code>ACTIVE</code> articles to ensure that only
   * published articles are included in the caches (e.g., Queries by Category stored in BrowseCache).
   *
  ...
  /**
   * Return unsorted map of Article objects with a <code>state</code> of <code>ACTIVE</code>
   * (meaning the articles have been published)
   * sorted by <code>date</code> and <code>article id</code>.
   * <br/>
   * It is necessary to query for <code>ACTIVE</code> articles to ensure that only
   * published articles are included in the caches (e.g., Queries by Date stored in BrowseCache).
   *
   * @return Active Article objects by category
   */
  private Map<String, Set<URI>> fetchArticlesByCategory() {
    ...
    query.setParameter("st", Article.STATE_ACTIVE);
  ...
  private Years loadArticleDates() {
    Query query = session.createQuery("select a.dublinCore.date from Article a where a.state = :st;");
    query.setParameter("st", Article.STATE_ACTIVE);
  ...
  /**
   * Get the URIs for all of the Articles with a <code>state</code> of <code>ACTIVE</code> (meaning
   * the articles have been published) that were published between the <code>start date</code>
  ...
  private List<URI> loadArticlesByDate(final Calendar startDate, final Calendar endDate,
    ...
    StringBuilder queryString = new StringBuilder("select a.id articleId, date from Article a "
        + " where date := a.dublinCore.date and a.state = :st and gt(date, :sd) and lt(date, :ed)");
    int counter = 1;
