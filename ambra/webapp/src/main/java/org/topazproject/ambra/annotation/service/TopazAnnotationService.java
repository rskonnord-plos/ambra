 ...
 */
public class TopazAnnotationService extends BaseTopazAnnotationService implements AnnotationService {
  ...
  private static final Logger    log  = LoggerFactory.getLogger(TopazAnnotationService.class);
  ...
  throws ParseException, URISyntaxException {
    ...
    while (annotationResults.next()) {
      // FIXME: we should apply XACML access check here
   ...
   */
  @Transactional(rollbackFor = { Throwable.class })
  public void updateBodyAndContext(String id, String body, String context)
    throws OtmException, SecurityException, IllegalArgumentException, UnsupportedEncodingException {

    pep.checkAccess(AnnotationsPEP.UPDATE_ANNOTATION, URI.create(id));

    ArticleAnnotation a = session.get(ArticleAnnotation.class, id);
    if (a == null)
      throw new IllegalArgumentException("invalid annotation id: " + id);

    AnnotationBlob blob = a.getBody();
    blob.setBody(body.getBytes(getEncodingCharset()));
   ...
   *         is returned
   *
   * @throws org.topazproject.otm.OtmException if some error occurred
   * @throws SecurityException if a security policy prevented this operation
   */
  @Transactional(readOnly = true)
  ...
  public String convertAnnotationToType(final String srcAnnotationId,
    ...
    copyBeanProperties(srcAnnotation, newAn);

    if (newAn.getBody() == null) {
      StringBuilder errorMessage = new StringBuilder();
      errorMessage.append("Copy properties failed for <")
                  .append(srcAnnotation.getId().toString())
                  .append(">\nsrcAnnotation is of type ")
                  .append(srcAnnotation.getClass().getName())
                  .append("\nProperties:\n");
      PropertyDescriptor[] properties = PropertyUtils.getPropertyDescriptors(srcAnnotation);
      for (PropertyDescriptor property : properties) {
        errorMessage.append(property.getName())
                    .append(" is ")
                    .append(property.getPropertyType().getName())
                    .append('\n');
      }

      log.error(errorMessage.toString());

      newAn.setBody(srcAnnotation.getBody());

      if (newAn.getBody() == null)
          throw new ApplicationException("Copying of body property failed. " + newAn.getId().toString()
              + " has NULL body");
    }
    ...
    if (newAn instanceof FormalCorrection) {
      TopazAnnotationUtil.createDefaultCitation((FormalCorrection)newAn, getArticleCitation(newAn), session);

    } else if (newAn instanceof Retraction) {
      TopazAnnotationUtil.createDefaultCitation((Retraction)newAn, getArticleCitation(newAn), session);
    }
  ...
  /*
   * Creates a new instance of PropertyUtilsBean every time because we don't want class property caching.
   * See ticket #1108
   */
  private void copyBeanProperties(ArticleAnnotation srcAnnotation, ArticleAnnotation newAnnnotation)
      throws Exception {

    if (log.isDebugEnabled())
      log.debug("Copying properties from instance of " + srcAnnotation.getClass().getName() +
          " to instance of " + newAnnnotation.getClass().getName());
    PropertyUtils.copyProperties(newAnnnotation, srcAnnotation);
  }
  ...
  private Citation getArticleCitation(ArticleAnnotation annotation) {
    Article article = session.get(Article.class, annotation.getAnnotates().toString());
    if (article == null)
      throw new OtmException("Article " + annotation.getAnnotates() + " not found");
