...
import org.topazproject.ambra.article.service.IngestArchiveProcessor;
import org.topazproject.ambra.article.service.Ingester;
import org.topazproject.ambra.models.Article;
import org.topazproject.ambra.models.Syndication;
import org.topazproject.ambra.struts2.ManualTransactionManagement;
...
public class AdminTopAction extends BaseAdminActionSupport {
   ...
   * org.topazproject.ambra.struts2.TransactionInterceptor} there would be a transaction wrapped around the whole
   * request, and a failure of one article to ingest would cause them all to fail.
   * <p/>
   * See the javadoc on that the {@link org.topazproject.ambra.struts2.TransactionInterceptor} for the refactoring
    ...
    if (filesToIngest != null) {
      for (String file : filesToIngest) {
        ...
        try {
          ...
          article = (Article) template.execute(new TransactionCallback() {
            @Override
          ...
          log.error("error committing transaction during ingest of " + filename, e);
          addActionError("Error ingesting: " + filename + " - " + getMessages(e));
        }
        if (article != null) {
          try {
            documentManagementService.generateCrossrefInfoDoc(
                ingestArchiveProcessor.extractArticleXml(new ZipFile(archive)), article.getId());
            documentManagementService.generateIngestedData(archive, article);
            log.info("Successfully ingested archive '" + filename + ", stored article " + article.getId());
  ...
  private boolean setCommonFields() {
    ...
    try {
      ...
      if (orderBy.equals("doi asc")) {
        publishableArticles = articlePersistenceService.getPublishableArticles(
            getJournal().geteIssn(), ArticlePersistenceService.ORDER_BY_FIELD_ARTICLE_DOI, true);
      } else if (orderBy.equals("doi desc")) {
        publishableArticles = articlePersistenceService.getPublishableArticles(
            getJournal().geteIssn(), ArticlePersistenceService.ORDER_BY_FIELD_ARTICLE_DOI, false);
      } else if (orderBy.equals("pubdate asc")) {
        publishableArticles = articlePersistenceService.getPublishableArticles(
            getJournal().geteIssn(), ArticlePersistenceService.ORDER_BY_FIELD_DATE_PUBLISHED, true);
      } else if (orderBy.equals("pubdate desc")) {
        publishableArticles = articlePersistenceService.getPublishableArticles(
            getJournal().geteIssn(), ArticlePersistenceService.ORDER_BY_FIELD_DATE_PUBLISHED, false);
      }
      ...
      for (Article article : publishableArticles) {
        syndicationMap.put(article.getId().toString(), syndicationService.querySyndication(article.getId().toString()));
      }
