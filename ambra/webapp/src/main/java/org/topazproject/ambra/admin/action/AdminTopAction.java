    ...
    if (filesToIngest != null) {
      for (String file : filesToIngest) {
        ...
        try {
          ...
          Article article = (Article) template.execute(new TransactionCallback() {
            @Override
  ...
  private boolean setCommonFields() {
    ...
    try {
      ...
      if (orderBy.equals("doi asc")) {
        ...
        publishableArticles = articlePersistenceService.getPublishableArticles(
            getJournal().geteIssn(), ArticlePersistenceService.ORDER_BY_FIELD_DATE_CREATED, true);
      } else if (orderBy.equals("pubdate desc")) {
        publishableArticles = articlePersistenceService.getPublishableArticles(
            getJournal().geteIssn(), ArticlePersistenceService.ORDER_BY_FIELD_DATE_CREATED, false);
      }
