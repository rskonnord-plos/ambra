...
<!-- 10/8/09: viewnlm contains a bunch of informational comments (module, system, purpose, contents, change history, etc). we added one line (13a) to contents. -->
<!--              13a) Fix long words                              -->
...
<!-- 10/8/09: viewnlm 2.3 added new prefixes to the exclude-result-prefixes. use 2.3 changes? -->
...
<xsl:stylesheet version="2.0"
    ...
    <!-- 10/23/09: need to add absolute path once directory locations are decided -->
    ...
    <!--  10/8/09: viewnlm contains commented-out notes -->
    ...
    <!-- 10/13/09: plos-specific template -->
    <!-- 10/13/09: used for initials in [author line? citation? both?] -->
    ...
    <!-- 10/13/09: plos-specific template -->
    <!-- 10/13/09: used for initials in [author line? citation? both? adding dash to initials in hyphenated names?] -->
      ...
      <xsl:for-each select="front/article-meta">
        ...
        <p>
         ...
         <!-- 10/29/09: article title -->
          ...
          <!-- 10/29/09: any reason we can't replace this bug fix with a call to endsWithPunctuation template (don't add citation period if article-title ends with punctuation)? -->
        ...
        <p>
          <xsl:choose>
            <xsl:when test="$dtd-version &lt; 2.3">
              <xsl:choose>
                <!-- Modified to output the word "copyright" for the header if the expression "Open-Access License" appears anywhere in the copyright-statement. -->
    ...
    <!-- 10/26/09: plos comment, not sure when added:
                  Added the following template rules to correctly copy
                  and map different markup within glossary definitions -->
        ...
        <p class="authors" xpathLocation="noSelect">
          <xsl:for-each select="contrib-group/contrib[@contrib-type='author']">
            <xsl:choose>
              ...
              <!-- 10/27/09: delete this test, or comment it out? -->
              <!-- (plos comment, not sure when added): the following test for @xlink probably is not used any more and can be removed -->
              <!-- 4/15/10: added dc & foaf, brought up to date with current production. we do use this, at least for the dc & foaf. -->
              ...
              <!-- 2/22/10: leave commented out for now, don't use until 3.0 upgrade -->
    ...
    <!-- 10/19/09: plos-specific template. keep it, or delete and deal with spaces when xml is incorrect? -->
    <!-- Hack to remove leading and trailing space from addr-line elements that were incorrectly written this way. Ultimately, XML files should be cleaned up rather than this hack. -->
    ...
    <!-- 10/19/09: viewnlm contains sections 11 (make-post-publication) and 12 (make-end-metadata). plos doesn't call section 12 at all? -->
   ...
   <xsl:template match="p">
    ...
    <!-- 10/21/09: viewnlm contains section 16 (display-quote), section 17 (speech and its internals, section 18 (statement and its internals), and section 19 (verse-group and its internals) -->
    ...
    <!-- 10/21/09: viewnlm 2.3 added text & copy-of to template tex-math -->
    ...
    <!-- 10/22/09: viewnlm 2.3 changed template self-uri: moved span, added choose & otherwise. -->

    <!-- 2/23/10: viewnlm contains copyright-statement|copyright-year|copyright-holder. we don't use it -->

    <!-- 10/22/09: viewnlm 2.3 contains new template license. we don't currently use it (and won't until 3.0 upgrade) -->
    ...
    <!-- 10/22/09: viewnlm 2.3 contains new template custom-meta-wrap. we don't use it -->
    ...
    <!-- 11/3/09: viewnlm contains the next few templates (surname mode contrib, suffix mode contrib, degrees mode contrib) -->
    ...
    <!-- 11/3/09: viewnlm contains templates (aff-outside-contrib, on-behalf-of, role, email, author-notes mode front, author-notes title) -->
    ...
    <!-- 2/11/10: plos modifications (revert to nlm v2 version; 2.3 version causes extra period) -->
    ...
    <!-- 11/4/09: viewnlm 2.3 contains new template (list/title) -->

    <!-- 11/4/09: viewnlm 2.3 contains new template (def-list/title). we override in section 46 -->
    ...
    <!-- 11/4/09: viewnlm contains new template (license) -->
    ...
    <!-- 11/5/09: viewnlm contains template (article-title mode conf) -->
    ...
    <!-- 11/5/09: viewnlm 2.3 contains 2 new templates (formatting elements for citations) -->
    ...
    <xsl:template match="issue" mode="none">
      ...
      <xsl:choose>
        <!-- 5/7/10: added test for elocation-id. 5/25: temporarily commented out until testing diffs are done -->
    ...
    <!-- 5/7/10: plos-specific template. 5/25: temporarily commented out for diff testing -->
    <!--<xsl:template match="elocation-id" mode="none">
	   <xsl:apply-templates/>
       <xsl:text>. </xsl:text>
    </xsl:template>    -->

    <!-- 11/5/09: viewnlm contains template (publisher-loc mode none) -->
    ...
    <!-- 11/5/09: viewnlm contains template (collab mode none) -->
   ...
   <!-- 11/5/09: viewnlm contains templates (access-date mode none, season mode none) -->
    ...
    <!-- 11/5/09: viewnlm 2.3 contains new template (lpage mode write) -->
    ...
    <!-- 2/24/10: viewnlm 2.3 contains new template (comment mode none) -->
