...
<#--
  The collection of supported sorts:
-->
<#assign sorts = [
  [ "Relevance", "score desc, publication_date desc, id desc"],
  [ "Date, newest first", "publication_date desc, score desc, id desc"],
  [ "Date, oldest first", "publication_date asc, score desc, id desc"]
]>

<#--
  Defined page sizes
-->
<#assign pSizes = [ 10, 25, 50]>
            ...
            <select name="sort" id="sortPicklist">
              <#list sorts as sortItem>
                <#if ((!sort?? || (sort?? && sort == "")) && (sortItem_index == 0))>
                  <option selected value="${sortItem[1]}">${sortItem[0]}</option>
                <#else>
                  <#if (sort?? && (sort == sortItem[1]))>
                    <option selected value="${sortItem[1]}">${sortItem[0]}</option>
                  <#else>
                    <option value="${sortItem[1]}">${sortItem[0]}</option>
                  </#if>
                </#if>
              </#list>
            </select>
        ...
        <dl id="authorFacet" class="facet">
          ...
          <#list resultsSinglePage.authorFacet as f>
            <#if f_index < max_authors>
              <dd><a href="${advancedSearchURL}?unformattedQuery=author%3A%22${f.name?url}%22&from=authorLink&sort=${sorts[1][1]?url}">
              ${f.name}</a></dd>
            </#if>
          </#list>
        </dl>
        ...
        <dl id="editorFacet" class="facet">
          ...
          <#list resultsSinglePage.editorFacet as f>
            <#if f_index < max_editors>
              <dd><a href="${advancedSearchURL}?unformattedQuery=editor%3A%22${f.name?url}%22&from=editorLink&sort=${sorts[1][1]?url}">
              ${f.name}</a></dd>
            </#if>
          </#list>
        </dl>
        ...
        <dl id="institutionsFacet" class="facet">
          ...
          <#list resultsSinglePage.institutionFacet as f>
            <#if f_index < max_institutions>
              <dd><a href="${advancedSearchURL}?unformattedQuery=affiliate%3A%22${f.name?url}%22&from=institutionLink&sort=${sorts[1][1]?url}">
              ${f.name}</a></dd>
            </#if>
          </#list>
        </dl>
