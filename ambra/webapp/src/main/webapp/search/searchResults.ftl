  ...
  <#if (totalPages gt 1) >
    <#if (startPage gt 0) >
      <#if limitToJournalAsString??>
        <@s.url id="prevPageURL" action="simpleSearch" namespace="/search2" startPage="${startPage - 1}" pageSize="${pageSize}" query="${query}" journalOpt="${journalOpt}" limitToJournal="${limitToJournalAsString}" includeParams="none"/>
      <#else>
        <@s.url id="prevPageURL" action="simpleSearch" namespace="/search2" startPage="${startPage - 1}" pageSize="${pageSize}" query="${query}" journalOpt="${journalOpt}" includeParams="none"/>
      </#if>
    ...
    <#list max(1, startPage - 9)..min(totalPages, startPage + 11) as pageNumber>
      ...
      <#else>
        <#if limitToJournalAsString??>
          <@s.url id="searchPageURL" action="simpleSearch" namespace="/search2" startPage="${pageNumber - 1}" pageSize="${pageSize}" query="${query}" journalOpt="${journalOpt}" limitToJournal="${limitToJournalAsString}" includeParams="none"/>
        <#else>
          <@s.url id="searchPageURL" action="simpleSearch" namespace="/search2" startPage="${pageNumber - 1}" pageSize="${pageSize}" query="${query}" journalOpt="${journalOpt}" includeParams="none"/>
        </#if>
    ...
    <#if hasMore == 1>
      <#if limitToJournalAsString??>
        <@s.url id="nextPageURL" action="simpleSearch" namespace="/search2" startPage="${startPage + 1}" pageSize="${pageSize}" query="${query}" journalOpt="${journalOpt}" limitToJournal="${limitToJournalAsString}" includeParams="none"/>
      <#else>
        <@s.url id="nextPageURL" action="simpleSearch" namespace="/search2" startPage="${startPage + 1}" pageSize="${pageSize}" query="${query}" journalOpt="${journalOpt}" includeParams="none"/>
      </#if>
...
<div id="content" class="static">
  ...
  <#--  <p>Debug: noOfResults: ${noOfResults}, totalNoOfResults: ${totalNoOfResults}, startPage: ${startPage}, pageSize: ${pageSize}, hasMore: ${hasMore}</p> -->
    ...
    <div id="searchMore">
      ...
      <!--  TODO: This "action" should be composed dynamically, but is here hard-coded to make the old (Mulgara-based) search work.
            TODO: The next line should read: form name="reviseQuery" action="${searchURL}" method="get"
      -->
      <form name="reviseQuery" action="/ambra-webapp/search2/simpleSearch.action" method="get">
        <@s.hidden name="noSearchFlag" value="set" />
        <@s.hidden name="creator" />
        <@s.hidden name="authorNameOp" />
        <@s.hidden name="textSearchAll" />
      ...
      <ul>
        <#list searchResults as hit>
          <li>
            ...
            <#if journalOpt?? && journalOpt == "some" && limitToJournal?? && limitToJournal?size == 1 && limitToJournal?first == journalContext>
              <#if hit.journalKey != journalContext>
                <strong><em>${hit.journalTitle}</em></strong><em>:</em>
              </#if>
            <#else>
              <strong><em>${hit.journalTitle}</em></strong><em>:</em>
            </#if>
            ${hit.articleTypeForDisplay}, published ${hit.date?string("dd MMM yyyy")} | ${hit.uri?replace("info:doi/", "doi:")}
          </li>
        </#list>
      </ul>
      <div class="resultsTab"><@renderSearchPaginationLinks totalPages, hasMore/></div>
