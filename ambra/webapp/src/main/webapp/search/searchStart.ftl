...
<#list journals as journal>
                <li><input id="limitToJournal_${journal.key}" name="limitToJournal" value="${journal.key}" type="checkbox" <@chkdlist tstr=journal.key strlist=(limitToJournal![])/>/>&nbsp;<label for="limitToJournal_${journal.key}">${journal.dublinCore.title}</label></li>
</#list>
              </ul>
            </fieldset>
          </li>
