    ...
    <ul class="syndHist">
        <#list syndicationHistory as syndicate>
            ...
            <#if syndicate.syndication.status == 'IN_PROGRESS'>
                <li class="inprogress">
                    <#if syndicate.syndication.target == 'EBSCO'>
                        Syndication to ${syndicate.syndication.target}
                        attempted on ${syndicate.syndication.statusTimestamp?datetime}<br/>
                        Syndication may take up to 24 hours.
                        <#else>
                            Syndication to ${syndicate.syndication.target}
                ...
                <@s.hidden name="article" value="${article}"/>
                    <input type="submit" value="Mark As Failed"
                           onClick="return confirmMarkAsFailed('${syndicate.syndication.statusTimestamp?datetime}','${syndicate.syndication.target}');"/>
                </@s.form>
                </li>
            </#if>
            ...
            <#if syndicate.syndication.submissionCount == 0>
                ...
                <#else>
                    ...
                    ${syndicate.syndication.submissionCount} times
                    </li>
            </#if>
    ...
    <#if finishedSyndications?size == 1>
        ...
        <@s.hidden name="article" value="${article}"/>
            <#if syndicate.syndication.status == 'PENDING'>
                ...
                <#else>
                    <input type="submit" value="Syndicate"
                           onClick="return confirmResyndicate('${syndicate.syndication.statusTimestamp?datetime}','${syndicate.syndication.target}');"/>
            </#if>
        </@s.form>
        </#list>
        <#else>
        ...
        <@s.hidden name="article" value="${article}"/>
            Syndicate to:
            <#list finishedSyndications as syndicate>
                <input type="checkbox" name="target" value="${syndicate.syndication.target}"
                       checked=true/> ${syndicate.syndication.target}<#if syndicate_has_next>, </#if>
