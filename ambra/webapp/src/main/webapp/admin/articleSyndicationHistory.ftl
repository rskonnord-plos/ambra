...
<#if syndicationHistory??>
    <ul class="syndHist">
        <#list syndicationHistory as syndicate>
            <li><strong>Article:</strong> ${syndicate.articleURI}</li>
            ...
            <#if syndicate.status == 'IN_PROGRESS'>
                <li class="inprogress">
                    <#if syndicate.target == 'EBSCO'>
                        Syndication to ${syndicate.target}
                        attempted on ${syndicate.statusTimestamp?datetime}<br/>
                        Syndication may take up to 24 hours.
                        <#else>
                            Syndication to ${syndicate.target}
                            is in progress as of ${syndicate.statusTimestamp?datetime}<br/>
                ...
                <@s.hidden name="article" value="${article}"/>
                    <input type="submit" value="Mark As Failed"
                           onClick="return confirmMarkAsFailed('${syndicate.statusTimestamp?datetime}','${syndicate.target}');"/>
                </@s.form>
                </li>
            </#if>
            <#if syndicate.status == 'SUCCESS'>
                <li class="success">Syndication Succeeded on
                ${syndicate.statusTimestamp?datetime}</li>
            </#if>
            <#if syndicate.status == 'FAILURE'>
                <li class="error">Syndication to ${syndicate.target} Failed on
                ${syndicate.statusTimestamp?datetime}
    ...
    <#if finishedSyndications?size == 1>
        ...
        <@s.hidden name="article" value="${article}"/>
            <#if syndicate.status == 'PENDING'>
                ...
                <#else>
                    <input type="submit" value="Syndicate"
                           onClick="return confirmResyndicate('${syndicate.statusTimestamp?datetime}','${syndicate.target}');"/>
            </#if>
        </@s.form>
        </#list>
        <#else>
