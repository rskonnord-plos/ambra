...
dojo.addOnLoad(function()
  {
    if(almHost != null && almHost != '' && almHost != 'alm.example.org') {
      var almService = new ambra.alm(almHost, journal);
...
function setCites(response, args)
{
  ...
  if (response.article.sources.length > 0) {
    ...
    var scopusOneURL;
    var scopusOneCount = 0;
    var scopusTwoURL;
    var scopusTwoCount = 0;
    ...
    for (var a = 0; a < response.article.sources.length; a++) {
      ...
      //Only list links that HAVE DEFINED URLS
      if (url) {
        //Gross Hack related to trac item #1260
        //TODO: This is very PLOS specific, this should be moved to an override
        if (response.article.sources[a].source == 'ScopusDOI' || response.article.sources[a].source == "ScopusEISSN") {
          scopusOneURL = url;
          scopusOneCount = response.article.sources[a].count;
        } else if (response.article.sources[a].source == 'ScopusISSN') {
          scopusTwoURL = url;
          scopusTwoCount = response.article.sources[a].count;
        } else {
          html = html + "<li><a href=\"" + url + "\">" + response.article.sources[a].source + " ("
              + response.article.sources[a].count + ")</a></li>";
        }
        numCitesRendered++;
      }
    }

    if (scopusOneCount > 0 || scopusTwoCount > 0) {
      if (scopusOneCount > 0 && scopusTwoCount > 0) {
        html = html + "<li>Scopus (" +
              "<a href=\"" + scopusOneURL + "\">" + scopusOneCount + "</a>, " +
              "<a href=\"" + scopusTwoURL + "\">" + scopusTwoCount + "</a>" +
              ")</li>";
      } else {
        if (scopusOneCount > 0) {
          html = html + "<li><a href=\"" + scopusOneURL + "\">Scopus ("
              + scopusOneCount + ")</a></li>";
        }

        if (scopusTwoCount > 0) {
          html = html + "<li><a href=\"" + scopusTwoURL + "\">Scopus ("
              + scopusTwoCount + ")</a></li>";
        }
      }
      ...
      if (scopusOneCount > 0 && scopusTwoCount > 0) {
        html = html + "<i>(For some articles, two links are provided to Scopus, because articles have " +
               "been cited either using their print page numbers or using their 'enumbers'. " +
               "Currently, Scopus separates these citatations into two groups, but is in the " +
               "process of consolidating the citations into a single group.)</i><br/><br/>";
      }
