...
public class ArticleMigrationTest extends BaseMigrationTest {
  ...
  public void compareParts(Article mySqlArticle, Article topazArticle) {
    ...
    assertFalse((mySqlParts.size() !=  topazParts.size()), "Topaz and MySQL have differing article parts size;" +
  ...
  public void compareCategories(Article sa, Article ta)
  {
    ...
    assertFalse((saCategories.size() !=  taCategories.size()), "Topaz and MySQL have differing category size;" +
          " article: " + sa.getId());
  ...
  public void compareRelatedArticles(Article sa, Article ta)
  {
    ...
    if(mySqlRels.size() == 0)
    {
      ...
      assertFalse((topazRels.size() !=  mySqlRels.size()), "Topaz and MySQL have differing related article size;" +
          " article: " + sa.getId());
  ...
  private static void compareLicenses(DublinCore mySqlDCore, DublinCore topazDCore, String msg)
  {
    ...
    if(mySqlDCore.getLicense().size() == 0) {
      ...
      assertFalse((topazLicenses.size() !=  mySqlLicenses.size()), "Topaz and MySQL have differing licenses  size;" +
          " article: " + mySqlDCore.getIdentifier());
  ...
  public void compareObjectInfoProperties(ObjectInfo mysqlObjectInfo, ObjectInfo topazObjectInfo)
  {
    ...
    Set<Representation> r1 = mysqlObjectInfo.getRepresentations();
    Set<Representation> r2 = topazObjectInfo.getRepresentations();
    assertMatchingSets(getRepresentationIds(r1),getRepresentationIds(r2),
