...
public class SchemaGenerator {
  ...
  public void generateSQL(Dialect dialect) {
    ...
    log.info("Generated schema in " + dialect + " dialect; output to " + new File(outputFile).getAbsolutePath());
  }
  ...
  public void createSchema(String jdbcUrl, Dialect dialect, String username, String password) {
    ...
    SchemaExport export = new SchemaExport(configuration);
    export.setDelimiter(";");
    log.info("Running schema creation script for " + jdbcUrl);
    export.execute(false, true, false, !updateSchema);
    log.info("Created database schema " +
        " in " + configuration.getProperty("connection.url"));
  }
